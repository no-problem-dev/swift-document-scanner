...
swift-document-scannerの新バージョンをリリースする手順を説明します。
...
```bash
# リリースブランチに切り替え（例: release/v0.4.0）
...
**重要**: バージョン番号は必ずブランチ名と一致させる（`release/v0.4.0` → `[0.4.0]`）
...
**このマージで自動的に実行されること:**
1. タグ `v0.4.0` が作成される
2. GitHub Release `v0.4.0` が作成される
3. 次のリリースブランチ `release/v0.4.1` が作成される
...
| 変更内容 | バージョン | 例 |
|---------|-----------|-----|
| バグ修正のみ | PATCH | 0.3.1 → 0.3.2 |
| 新機能追加（後方互換） | MINOR | 0.3.2 → 0.4.0 |
| 破壊的変更 | MAJOR | 0.4.0 → 1.0.0 |
...
### 追加
- **PerspectiveCorrection強化** - 台形補正の精度を向上させるアルゴリズムを追加

### 修正
- **DocumentCamera** - iOS 18でカメラプレビューが回転する問題を修正
```
